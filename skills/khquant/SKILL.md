...
| 级别 | 命令类型 | 行为 |
|------|---------|------|
| **自动执行** | 查询类：`kh doctor`、`kh data stats/list/info`、`kh result list/show`、`kh strategy list/info/validate`、`kh tool *`、`kh config show`、`kh version` | 直接通过 Bash 运行，展示结果 |
| **确认后执行** | 修改类：`kh data download/scan --fix/sync/export/repair`、`kh run`、`kh result report/compare`、`kh strategy create`、`kh config set/reset` | 先展示将要执行的命令，用 AskUserQuestion 确认后再执行 |
| **必须确认** | 危险类：`kh result clean`、`kh data repair`、`kh init`（会覆盖现有配置） | 明确告知风险，必须确认 |
...
| "下载数据"、"股票池"、"数据源"、"同步"、"数据缺口" | 数据管理 | `references/data-management.md` |
